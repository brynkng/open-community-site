...
export async function getActivePrograms(): Promise<Program[]> {
  ...
  // Dinner leads the brand everywhere (landing page first segment + top nav);
  // the rest keep their configured sortOrder. Stable sort preserves that order.
  return rows.sort(
    (a, b) => (a.kind === "dinner" ? 0 : 1) - (b.kind === "dinner" ? 0 : 1),
  );
}
...
export async function getProgramsByKind(
  kind: "dinner" | "ride" | "trip",
): Promise<Program[]> {
  return getDb()
    .select()
    .from(programs)
...
export async function defaultProgramIdForKind(
  kind: "dinner" | "ride" | "trip",
): Promise<number | null> {
